...
class Trainer(object):
    ...
    def drug_decoder_load(self, dictionary_name):
        ...
        with open(
            "DrugGEN/data/decoders/" + dictionary_name + "_" + self.drugs_name + ".pkl",
            "rb",
        ) as f:
            return pickle.load(f)
    ...
    def train(self):
        ...
        full_smiles = [
            line
            for line in open("DrugGEN/data/chembl_train.smi", "r").read().splitlines()
        ]
        drug_smiles = [
            line for line in open("DrugGEN/data/akt_train.smi", "r").read().splitlines()
        ]
        ...
        akt1_human_adj = (
            torch.load("DrugGEN/data/akt/AKT1_human_adj.pt")
            .reshape(1, -1)
            .to(self.device)
            .float()
        )
        akt1_human_annot = (
            torch.load("DrugGEN/data/akt/AKT1_human_annot.pt")
            .reshape(1, -1)
            .to(self.device)
            .float()
        )
    ...
    def inference(self):
        ...
        drug_smiles = [
            line for line in open("DrugGEN/data/akt_test.smi", "r").read().splitlines()
        ]
        ...
        with torch.inference_mode():
            ...
            for i, data in enumerate(self.inf_loader):
                ...
                with open(
                    "DrugGEN/experiments/inference/{}/inference_drugs.txt".format(
                        self.submodel
                    ),
                    "a",
                ) as f:
